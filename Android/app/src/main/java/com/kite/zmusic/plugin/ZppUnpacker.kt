...
internal object ZppUnpacker {
    fun unpack(zpp: File, destDir: File): UnpackResult {
        if (!zpp.isFile) return UnpackResult.Invalid(t("不是文件"))
        destDir.mkdirs()
        return try {
            ZipFile(zpp).use { zip -> unpackZip(zip, destDir) }
        } catch (e: ZipException) {
            destDir.deleteRecursively()
            UnpackResult.Invalid(t("无法打开归档"))
        } catch (e: IOException) {
            destDir.deleteRecursively()
            UnpackResult.Invalid(t("读取归档失败"))
        }
    }
    ...
    private fun unpackZip(zip: ZipFile, destDir: File): UnpackResult {
        ...
        if (raw.isEmpty()) return UnpackResult.Invalid(t("空包"))
        ...
        for (entry in raw) {
            ...
            if (prefix != null) {
                ...
                if (!rel.startsWith(prefix)) return UnpackResult.Invalid(t("包根不一致"))
            ...
            if (!PluginPackageRules.zipEntryOk(rel, directory)) {
                return UnpackResult.Invalid(t("非法路径或扩展名: %s", rel))
            }
        ...
        for ((entry, rel) in mapped) {
            ...
            if (!out.path.startsWith(destCanon.path + File.separator) && out != destCanon) {
                return UnpackResult.Invalid(t("路径穿越"))
            }
    ...
    fun validateExtracted(root: File): UnpackResult {
        ...
        if (!jsonFile.isFile) return UnpackResult.Invalid(t("缺少 plugin.json"))
        val readme = File(root, "README.md")
        if (!readme.isFile) return UnpackResult.Invalid(t("缺少 README.md"))
        val hasPng = File(root, "plugin.png").isFile
        val hasSvg = File(root, "plugin.svg").isFile
        if (!hasPng && !hasSvg) return UnpackResult.Invalid(t("缺少图标"))
        val text = runCatching { jsonFile.readText(Charsets.UTF_8) }.getOrNull()
            ?: return UnpackResult.Invalid(t("无法读取 plugin.json"))
        val manifest = PluginManifestParser.parse(text)
            ?: return UnpackResult.Invalid(t("plugin.json 无效"))
        val entryFile = File(root, manifest.entry)
        if (!entryFile.isFile) return UnpackResult.Invalid(t("缺少入口"))
