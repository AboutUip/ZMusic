...
fun SplashAccelSettingsPage(
    ...
    Column(
        ...
        Text(
            text = t("开启后，冷启动只播放 Xuan 四个字母聚齐的那一段。字母完整出现后会在这一幕停住，然后直接进入软件，不再变成 ZMusic，也不再出示下面那两句标语。默认关闭。"),
            style = TextStyle(
        ...
        Text(
            text = t("插件还在加载、没法按这个节奏进门时，会当作没开这项：完整动画照常播完；若仍要等插件，定格后仍会转圈提示正在加载插件。关闭后立即生效，下次冷启动开始按新选择播放。"),
            style = TextStyle(
                color = MainPalette.Secondary,
                fontSize = 13.sp,
                lineHeight = 20.sp,
