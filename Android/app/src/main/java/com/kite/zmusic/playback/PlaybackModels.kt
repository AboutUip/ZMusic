...
data class PlaybackUiState(
    ...
    /** 心动模式：以歌单+种子曲拉智能列表，顺序连播并补歌，不灌回原歌单。 */
    ...
    fun withLyricPack(pack: LyricPack): PlaybackUiState = copy(
        ...
        translatedWordLyricLines = pack.wordTranslated,
    )
    ...
    fun withHydratedPeeks(): PlaybackUiState {
        ...
        val next = when (playbackMode) {
            PlaybackMode.ORDER, PlaybackMode.REPEAT_ONE ->
                queue.getOrNull(index + 1) ?: if (radioActive) null else queue.firstOrNull()
